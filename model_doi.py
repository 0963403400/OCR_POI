from custom_keras_layers_doi import *
from datetime import datetime
from dataset import Dataset
from hparams_doi import hparams
...
class Model(object):
    ...
    def create_model_train(self):
        ...
        # print('self.train_dataset.dataset',self.train_dataset.dataset)
        # print('self.valid_dataset.dataset',self.valid_dataset.dataset)
    ...
    def create_model_inference(self):
        ### create model
        # create conv layer
        self.conv_layer = ConvBaseLayer(hparams)
        # create attention + RNN layer
    ...
    def train_step(self, batch_input, batch_target):
        ...
        with tf.GradientTape() as tape:
            ...
            # dec_hidden = tf.cast(self.decoder.initial_hidden_state(current_batch_size), tf.float32)
    ...
    def inference(self, image):
        ...
        print('input shapes: ',dec_input.shape, dec_hidden.shape, conv_out.shape)
    ...
    def train(self):
        ...
        for epoch in range(self.last_epoch, hparams.max_epochs):
            ...
            for batch, (batch_input, batch_target, batch_text) in enumerate(self.train_dataset.dataset):
            ...
            sum_char = 0
            sum_str = 0
            for batch, (batch_input, batch_target, batch_text) in enumerate(self.valid_dataset.dataset):
                batch_true_char, batch_true_str = self.evaluate(batch_input, batch_target)
                cnt_true_char += batch_true_char
                cnt_true_str  += batch_true_str
                sum_char += batch_input.shape[0] * hparams.max_char_length
